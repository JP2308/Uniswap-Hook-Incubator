...
contract CSMM is BaseHook {
    ...
    function _beforeSwap(
        ...
        */
        ...
        if (isExactInput) {
            // User specified input (negative amountSpecified)
            absInputAmount = int128(-params.amountSpecified); // e.g. 100

            // Output is derived from the input (1:1 spot) then reduced by fee
            // Compute output from input, then apply 3% fee on output
            absOutputAmount = int128(uint128((uint256(uint128(absInputAmount)) * 97) / 100)); // user receives 97%

            beforeSwapDelta = toBeforeSwapDelta(
                absInputAmount,    // specified (input) : credit to hook
                -absOutputAmount   // unspecified (output): debit from hook
            );
        } else {
            // User specified output (positive amountSpecified)
            absOutputAmount = int128(params.amountSpecified); // e.g. 100

            // Input required is derived from desired output; increase by 3% fee on input
            absInputAmount = int128(uint128((uint256(uint128(absOutputAmount)) * 103) / 100)); // user pays 103%

            // For exact output, specified currency = output, so specified delta should be -absOutput
            // and unspecified delta should be +absInput
            beforeSwapDelta = toBeforeSwapDelta(
                -absOutputAmount,  // specified (output): hook owes this to PM
                absInputAmount     // unspecified (input): PM owes this to hook (user will send it)
            );
        }
